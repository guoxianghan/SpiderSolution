...
{
    //public delegate void DownloadCompleteCallBack(object sender, System.IO.MemoryStream memorystream);
    /// <summary>
    ...
    public class HttpServer //: HttpWebRequest
    {
        ...
        public virtual HttpResult GetHttpResult()
        {
            ...
            if (!string.IsNullOrEmpty(this.AcceptEncoding))
                this.HeaderCollection.Set("Accept-Encoding", this.AcceptEncoding);
            if (!string.IsNullOrEmpty(this.AcceptLanguage))
                this.HeaderCollection.Set("Accept-Language", this.AcceptLanguage);
            if (!string.IsNullOrEmpty(this.Origin))
                this.HeaderCollection.Set("Origin", this.Origin);
            if (!string.IsNullOrEmpty(this.UACPU))
                this.HeaderCollection.Set("UA-CPU", this.UACPU);
            if (!string.IsNullOrEmpty(this.x_requested_with))
                this.HeaderCollection.Set("x-requested-with", this.x_requested_with);
            //if (this.Method.ToLower() == "get")
            //{ this.Content_Length = 0L; }
            //else { this.Content_Length = 1L; }
            HttpResult result = httpHelper.GetHtmlResult(this, out err);
            return result;
        }

        public virtual HttpResult PostHttpResult()
        {
            string err;
            this.Method = "post";
